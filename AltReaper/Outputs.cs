...
namespace Reaper
{
    internal class Outputs
    {
        ...
        public static bool MailOutput(String recipient, String subjectLine, String[] content, String[] langValue, String[] config)
        {
            //Set salutation
            Console.Write($"\n{langValue[20]}\n>");
            ...
            //Set smtp config
            var smtpClient = new SmtpClient(config[3], int.Parse(config[4]))
            {
                Credentials = new NetworkCredential(config[1], config[2]),
                EnableSsl = true,
            };
            ...
            if (config[5] == langValue[17]) { } 
            else { mailMessage.Bcc.Add(config[5]); }
